#ifndef ASLAM_DOUBLESPHERE_DISTORTION_H_
#define ASLAM_DOUBLESPHERE_DISTORTION_H_
...
/// \class DoubleSphereDistortion
/// \brief An implementation of the double sphere distortion model for pinhole cameras.
class DoubleSphereDistortion : public aslam::Cloneable<Distortion, DoubleSphereDistortion> {
 public:
  /** \brief Number of parameters used for this distortion model. */
  enum { kNumOfParams = 2 };

  enum { CLASS_SERIALIZATION_VERSION = 1 };
  ASLAM_POINTER_TYPEDEFS(DoubleSphereDistortion);

  //////////////////////////////////////////////////////////////
  /// \name Constructors/destructors and operators
  /// @{

  /// \brief DoubleSphereDistortion Ctor.
  /// @param[in] distortionParams Vector containing the distortion parameter. (dim=2: k1, k2)
  explicit DoubleSphereDistortion(const Eigen::VectorXd& distortionParams);

  /// \brief Convenience function to print the state using streams.
  friend std::ostream& operator<<(std::ostream& out, const DoubleSphereDistortion& distortion);

 public:
  /// Copy constructor for clone operation.
  DoubleSphereDistortion(const DoubleSphereDistortion&) = default;
  void operator=(const DoubleSphereDistortion&) = delete;
 ...
 public:
  ...
  static DoubleSphereDistortion::UniquePtr createTestDistortion() {
    Eigen::VectorXd params(2); params << 0.1, 0.5;
    return DoubleSphereDistortion::UniquePtr(new DoubleSphereDistortion(params));
  }
...
#endif /* ASLAM_DOUBLESPHERE_DISTORTION_H_ */
